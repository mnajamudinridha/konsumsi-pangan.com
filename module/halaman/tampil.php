...
if (empty($_SESSION['login']) || $_SESSION['login'] != 'admin') {
  ...
  <ul class='pagination'>";
        if ($nohalaman > 1) {
            echo "<li><a href='".$_SERVER['PHP_SELF'].'?menu=halaman&halaman='.($nohalaman - 1).'&cari='.$cari."' aria-label='Previous'>
        ...
        for ($halaman = 1; $halaman <= $jumlahhalaman; ++$halaman) {
            if ((($halaman >= $nohalaman - 3) && ($halaman <= $nohalaman + 3)) || ($halaman == 1) || ($halaman == $jumlahhalaman)) {
                ...
                if ($halaman == $nohalaman) {
                    ...
                    echo "<li><a href='".$_SERVER['PHP_SELF'].'?menu=halaman&halaman='.$halaman.'&cari='.$cari."'>".$halaman.'</a></li>';
                }
        ...
        if ($nohalaman < $jumlahhalaman) {
            echo "<li>
            <a href='".$_SERVER['PHP_SELF'].'?menu=halaman&halaman='.($nohalaman + 1).'&cari='.$cari."' aria-label='Next'>
