...
(function(exports) {
	...
	var html = "", baseDir;
	var modules = {}, exposedModuls = [], moduleCount = 0;
	...
	function resolve(id) {
		if (exports.privateModuls && id in exports.privateModuls) {
			return;
		}
	...
	function define(id, dependencies, definition) {
		...
		if (--moduleCount === 0) {
			for (i = 0; i < exposedModuls.length; i++) {
				register(exposedModuls[i]);
			}
		}

		// Expose private modules for unit tests
		if (exports.AMDLC_TESTS) {
			privateModuls = exports.privateModuls || {};

			for (id in modules) {
				privateModuls[id] = modules[id];
			}

			for (i = 0; i < exposedModuls.length; i++) {
				delete privateModuls[exposedModuls[i]];
			}

			exports.privateModuls = privateModuls;
		}
